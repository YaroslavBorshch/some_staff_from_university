...
def mean_geom_sw(y_noisy, weights):
    ...
    return [np.prod([y_padded[i + j] ** w for j, w in enumerate(weights)]) for i, _ in enumerate(y_noisy)]
...
def random_search(y_noisy, weights_generator, tries, l):
    weights = [next(weights_generator) for _ in range(tries)]
    y_filtered = [mean_geom_sw(y_noisy, w) for w in weights]
    noise_c = [manhattan_noise_c(y_a) for y_a in y_filtered]
    diff_c = [manhattan_diff_c(y_a, y_noisy) for y_a in y_filtered]
    lin_conv = [l * n_c + (1. - l) * d_c for n_c, d_c in zip(noise_c, diff_c)]
    ind = np.argmin(lin_conv, axis=0)
    return [weights[ind], noise_c[ind], diff_c[ind], lin_conv[ind]]


def find_weights(y_noisy, sw_size, tries, arrangement, seed=42):
    res = []
